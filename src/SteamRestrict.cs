...
public class PluginConfig : BasePluginConfig
{
    ...
    [JsonPropertyName("BlockUnconfiguredProfile")]
    public bool BlockUnconfiguredProfile { get; set; } = false;
...
public class SteamRestrictPlugin : BasePlugin, IPluginConfig<PluginConfig>
{
    ...
    public readonly HttpClient Client = new HttpClient { Timeout = TimeSpan.FromSeconds(10) };
    ...
    public override void Load(bool hotReload)
    {
        ...
        {
            _databaseService = new DatabaseService(Config.DatabaseSettings);
            Task.Run(async () =>
            {
                try
                {
                    await _databaseService.EnsureTablesExistAsync();
                    await _databaseService.PurgeExpiredSavesAsync();
                }
                catch (Exception e)
                {
                    Logger.LogError($"Failed to connect to database or create tables. Player caching will not work. Error: {e.Message}");
        ...
        if (hotReload)
        {
            ...
            foreach (var player in Utilities.GetPlayers().Where(m => m.Connected == PlayerConnectedState.Connected && !m.IsHLTV && !m.IsBot && m.SteamID.ToString().Length == 17))
            {
    ...
    private void OnPlayerConnectFull(CCSPlayerController player)
    {
        ...
        {
            int retries = 0;
            g_hTimer[player.Slot] = AddTimer(3.0f, () =>
            {
                if (player.AuthorizedSteamID != null)
                {
                    g_hTimer[player.Slot]?.Kill();
                    g_hTimer[player.Slot] = null;
                    OnPlayerConnectFull(player);
                    return;
                }
        ...
        Task.Run(async () =>
        {
            try
            {
                ...
                {
                    if (await _databaseService.IsSteamIdAllowedAsync(authorizedSteamID))
                    {
            ...
            {
                Logger.LogError($"Unhandled error during connect check for {authorizedSteamID}: {e.Message}");
            }
        });
    }
    ...
    private void CheckUserViolations(nint handle, ulong authorizedSteamID)
    {
        ...
        Task.Run(async () =>
        {
                ...
                Server.NextWorldUpdate(() =>
                {
                    ...
                    if (player?.IsValid == true && userInfo != null)
                    {
                        ...
                        if (IsRestrictionViolated(player, userInfo))
                        {
                            ...
                            {
                                if (Config.KickDelay > 0)
                                {
                                    int playerSlot = player.Slot;
                                    g_iWarnTime[playerSlot] = Config.KickDelay;
                                    player.PrintToChat($" {ChatColors.Silver}[ {ChatColors.Lime}SteamRestrict {ChatColors.Silver}] {ChatColors.LightRed}You do not meet the minimum requirements. You will be kicked in {Config.KickDelay} seconds.");
                        ...
                        else if (_databaseService != null)
                        {
                            ...
                            if (steamID != 0)
                            {
                                Task.Run(async () =>
                                {
                                    try
                                    {
                                        await _databaseService.AddAllowedUserAsync(steamID, Config.DatabaseSettings.TablePurgeDays);
                                    }
                                    catch (Exception e)
                                    {
                                        Logger.LogError($"Failed to add allowed user {steamID} to database: {e.Message}");
                                    }
                                });
                            }
                        }
            ...
            {
                Logger.LogError($"Unhandled error during violation check for {authorizedSteamID}: {e.Message}");
            }
        });
    }
    ...
    private bool IsRestrictionViolated(CCSPlayerController player, SteamUserInfo userInfo)
    {
        ...
        if (Config.BlockUnconfiguredProfile && !(playerBypassConfig?.BypassUnconfiguredProfile ?? false) && !userInfo.IsProfileConfigured)
            return true;
